...
def collect(now: datetime | None = None) -> tuple[list[str], list[dict]]:
    ...
    for stop in load_watchlist_stops():
        ...
        raw_path = BASE_DIR / "data/bronze/vbb_departures" / now.strftime("%Y/%m/%d") / f"{stop['stop_id']}_{now.strftime('%H%M%S')}.json.gz"
    ...
    silver_path = BASE_DIR / "data/silver/departure_observations" / f"{day}.jsonl"
    if rows:
