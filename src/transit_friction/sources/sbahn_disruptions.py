...
from transit_friction.storage import sha256_bytes
...
def parse_html(html: str) -> list[dict]:
    ...
    # Very basic defensive parsing for S-Bahn HTML
    # Look for list items or divs that have classes indicating disruptions
    for item in soup.find_all(["li", "div"], class_=lambda x: x and ("disruption" in x.lower() or "stoerung" in x.lower() or "message" in x.lower())):
...
def normalize_events(raw_events: list[dict], now: datetime) -> list[dict]:
    ...
    for raw in raw_events:
        ...
        category = "disruption"
        if "bau" in text or "ersatz" in text:
        ...
        import re
        lines = list(set(re.findall(r'\b(S\d{1,2})\b', raw["raw_text"])))
        
        for line in (lines if lines else [None]):
            title = f"S-Bahn Alert: {category.capitalize()}"
            eid = stable_event_id("sbahn_disruptions", category, line, None, title, now)
            normalized.append({
                "event_id": eid,
                "source": "sbahn_disruptions",
